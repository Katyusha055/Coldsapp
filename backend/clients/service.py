...
'''
Temporary tests i made for the functions
clients_data = [
...
def create_test():
    with connect() as conn:    
        ...
        for client in clients_data:
            response.append(rep.create_clients(conn, client))
...
def update_client_test():
    with connect() as conn:
        ...
        response = rep.update_clients(conn, '1', '2', data)
        print(response)

def delete_client_test():
    with connect() as conn:
        data = {'id': '2', 'user_id': '1'}
        response = rep.delete_clients(conn, data)
        print(response)

def get_client_by_phone_test():
    with connect() as conn:
        data = {'user_id': '1', 'phone': '1234567890'}
        response = rep.get_clients_by_phone(conn, data)
        print(response)

def get_client_by_id_test():
    with connect() as conn:
        data = {'id': '3', 'user_id': '1'}
        response = rep.get_clients_by_id(conn, data)
        print(response)
        '''